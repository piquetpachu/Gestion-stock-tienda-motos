...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!form) console.warn('productos.js: no se encontró #formProducto en el DOM.');
    if (!modalProductoEl) console.warn('productos.js: no se encontró #modalProducto en el DOM.');
        ...
        if (selProv && selProv.tomselect) {
          ...
          if (!v) selProv.tomselect.clear();
            ...
            if (![...selProv.options].some(o => o.value === String(v))) {
              selProv.tomselect.addOption({ value: String(v), text: String(v) });
            }
        ...
        if (selRubro && selRubro.tomselect) {
          ...
          if (!v) selRubro.tomselect.clear();
            ...
            if (![...selRubro.options].some(o => o.value === String(v))) {
              selRubro.tomselect.addOption({ value: String(v), text: String(v) });
            }
    ...
    function cargarProductos() {
      fetch(API_URL + 'productos')
        ...
        .catch(err => console.error('productos.js: error cargando productos', err));
    }
    ...
    window.editar = function(p) {
      ...
      if (selProv && selProv.tomselect && p.id_proveedor) {
        if (![...selProv.options].some(o => o.value === String(p.id_proveedor))) {
          selProv.tomselect.addOption({ value: String(p.id_proveedor), text: String(p.id_proveedor) });
        }
        selProv.tomselect.setValue(String(p.id_proveedor));
      }
      const selRubro = document.getElementById('id_rubro');
      if (selRubro && selRubro.tomselect && p.id_rubro) {
        if (![...selRubro.options].some(o => o.value === String(p.id_rubro))) {
          selRubro.tomselect.addOption({ value: String(p.id_rubro), text: String(p.id_rubro) });
        }
    ...
    window.borrar = function(id) {
      ...
      fetch(API_URL + 'borrar_producto/' + id, { method: 'DELETE' })
        ...
        .catch(err => console.error('productos.js: error borrando producto', err));
    };

    // Nuevo producto
    window.nuevoProducto = function() {
      if (form) form.reset();
      if (document.getElementById('id_producto')) document.getElementById('id_producto').value = '';
      ...
      const selRubro = document.getElementById('id_rubro');
      if (selRubro && selRubro.tomselect && !restored) selRubro.tomselect.clear();
    ...
    if (busqueda) busqueda.addEventListener('input', () => { paginaActual = 1; mostrarProductos(); });
    if (ordenarPor) ordenarPor.addEventListener('change', () => { paginaActual = 1; mostrarProductos(); });
...
async function cargarProveedores() {
  ...
  try {
    ...
    if (!select.tomselect) {
      new TomSelect('#id_proveedor', { placeholder: 'Seleccione proveedor' });
    } else {
      select.tomselect.clearOptions();
      proveedores.forEach(p => {
        select.tomselect.addOption({ value: String(p.id_proveedor), text: p.nombre });
      });
    }
  } catch (err) {
...
document.getElementById('btnNuevoRubro')?.addEventListener('click', () => {
  ...
  if (productoModalPrevio) bootstrap.Modal.getInstance(modalProductoEl)?.hide();
...
document.getElementById('form-nuevo-rubro')?.addEventListener('submit', async e => {
  ...
  await res.json();
  await cargarRubros();
  bootstrap.Modal.getInstance(document.getElementById('modalNuevoRubro'))?.hide();
